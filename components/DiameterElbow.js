...
export function DiameterElbow (props) {
    ...
    useEffect(() => {
        ...
        const listenerId = props.currentDiameter.addListener(({ value }) => {
        ...
        return () => {
            props.currentDiameter.removeListener(listenerId); // cleaning
        };
    }, []);
    ...
    return (
        <View key={"diam-elbows"} style={[ { width: width*0.19, flexDirection: "column", justifyContent: "space-between", backgroundColor: "transparent" } ]}>
            ...
            <View style={[ {paddingLeft: width*0.007, justifyContent: "space-between", alignItems: "flex-start", backgroundColor: "transparent"} ]}>
                <ReTextAnimated style={[ styles.datasTopBar ]} fontSize={props.sizeText} color={props.color} name={DATAS_PIPES[diameter][props.norme._value]} />
                <ReTextAnimated style={[ styles.datasTopBar ]} fontSize={props.sizeText} color={props.color} name={(props.idSettingsDiameter == 0 ? `DN ${DATAS_PIPES[diameter][0]}` : `${DATAS_PIPES[diameter][props.idSettingsDiameter]} "`)} />
            </View>
        </View>
    )
